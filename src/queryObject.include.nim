...
For scalar-reads/-writes the `Cache[K,V]`-object provides the member `buckets`. Given a Query-object with `q.buckeIdx` and a `q.slot` one can access the buckets' keys via ´lru.buckets[q.bucketIdx].ks[q.slotIdx]´ and values via ´lru.buckets[q.bucketIdx].vs[q.slotIdx]´.
...
In multi-threaded-mode, the Query-object contains a additional member `lockState`. Tt indicates that the bucket `bucketIdx` is locked=`true` or not=`false`. It is a copy of the current state of the atomic-spin-lock from the `lru.locks`-array. Since it happens so easily to forget about unlocking, this copy of the lock-state-of the bucket is checked before the Query-object will be released. Thus a `q.lockState=true` in the destructor must always be false and results in a fatal/abort during debug-mode.
